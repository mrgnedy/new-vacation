...
class StatisticsbyMonth extends StatefulWidget {

  final Function callback;

  const StatisticsbyMonth({Key key, this.callback}) : super(key: key);

  @override
  _StatisticsbyMonthState createState() => _StatisticsbyMonthState();
}

class _StatisticsbyMonthState extends State<StatisticsbyMonth> {
  ...
  void initState() {
    ...
    _modelReservations.getAllMonthReservationsRecent(context: context);
                      ...
                      switch (snapshot.connectionState) {
                        ...
                        default:
                          if (snapshot.hasError)
                           ...
                           {
                             x=0;
                             snapshot.data.data.reservations.forEach((re){
                               x+=re.price;
                             });
                             model.setTotal=x;
                             widget.callback(snapshot.data.data.reservations);
                             return Container(
                              color: Colors.white,
                              child:
